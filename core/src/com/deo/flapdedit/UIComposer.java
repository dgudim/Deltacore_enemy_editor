...
import static com.deo.flapdedit.DUtils.getBoolean;
import static com.deo.flapdedit.DUtils.getFloat;
import static com.deo.flapdedit.DUtils.putBoolean;
import static com.deo.flapdedit.DUtils.putFloat;
...
public class UIComposer {
    ...
    public void loadStyles(String... styleNames) {
        ...
        JsonEntry styles = new JsonEntry(new JsonReader().parse(Gdx.files.internal("styles.json")));
        
        Array<BitmapFont> fonts = new Array<>();
        String[] fontNames = styles.getStringArray(new String[]{"font2(old).fnt", "font2.fnt"}, "fonts");
    ...
    private void loadStyle(JsonEntry treeJson, String style, Skin textures, Array<String> dependencies, Array<BitmapFont> fonts) {
        ...
        if (!dependencies.contains(dependency, false)) {
            ...
            textures.addRegions(assetManager.<TextureAtlas>get(dependency));
        }
    ...
    public Table addSlider(String style, int min, int max, float step, final String text, final String postText, final String valueKey) {
        ...
        final Label textLabel = addText(text + String.format(Locale.ROOT, "%.1f", getFloat(valueKey)) + postText, (BitmapFont) assetManager.get("font2(old).fnt"), 0.48f);
